function Partials = PT_Algo_FM( mXdB, ploc, vploc, Parm )
...
%         Parm.MagCond: magnitude deviation
...
if ~isfield(Parm, 'MagCond'); MagCond = 4; else MagCond = Parm.MagCond; end
if ~isfield(Parm, 'numFrames'); numFrames = 1874; else numFrames = Parm.numFrames; end
...
for t = 1:numFrames
    ...
    if any(TracksStatus == 0)
        ...
        for c = 1:CurNumPeaks
            ...
            DiffMag = abs(pmag{t}(c)-PrevPeak(:,2));
            
            % Find intersect peaks
            idxs = (DiffFreq < FreqCond & DiffMag < MagCond);
